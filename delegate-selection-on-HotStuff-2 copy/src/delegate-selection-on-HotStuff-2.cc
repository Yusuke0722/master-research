/*
 * delegate-selection-on-HotStuff-2.cc
 *
...
#include "delegate-selection-on-HotStuff-2.h"

#define BUFFER_MAX 4096
#define LLAMBDA 20
#define SLAMBDA 5
#define WAIT_MAX 10
#define BLOCK_MAX 2048 * 1000
#define TX_SIZE 512
...
void Create::publish_message(uint32_t step, int sign) {
    ...
    int fromL = sign % CREATE_NUM, toL = (fromL + nL) % CREATE_NUM;
    int fromC = (fromL * round) % CREATE_NUM, toC = (fromC + nD) % CREATE_NUM;
    int fromD = (fromC * round) % CREATE_NUM, toD = (fromD + nD) % CREATE_NUM;
    ...
    if (isCommittee(getId() - 2, fromL, toL)) {
        if (step == 1) {
...
void Create::find_leader(uint32_t step) {
    int fromL = newBlock.sign % CREATE_NUM, toL = (fromL + nL) % CREATE_NUM;
    int fromC = (fromL * round) % CREATE_NUM, toC = (fromC + nD) % CREATE_NUM;
    bool isCommit = false;
    for (json m : messages) {
        int sendId = m["sender"];
        if (step == 2 && m["round"] == round && m["step"] == step - 1 && isCommittee(sendId, fromL, toL)) {
            if (fromL <= sendId && sendId < leadId || leadId < fromL && !(leadId < sendId && sendId < fromL)) {
                lead_sign = m["sign"];
                leadId = sendId;
            }
        } else if (step > 2 && m["round"] == round && m["step"] == step - 1 && m["sender"] == leadId) {
            isCommit = true;
        } else if (step > STEP_MAX && m["round"] == round && m["sender"] == leadId) {
            finish_round();
            return;
        }
    }
    if (step == 2 && leadId == fromL && isCommittee(getId() - 2, fromC, toC) || step > 2 && isCommit || simTime() - addingTime >= 2*SLAMBDA) {
...
void Create::next_step() {
    if (newBlock.step > STEP_MAX) {
        if (getId() - 2 == leadId) {
            send_to_id(newBlock.step, lead_sign, 0, CREATE_NUM);
        }
        finish_round();
        return;
    }
...
void Concentrate::handleMessage(cMessage *msg) {
    ...
    for (int i = 0; i < committee.size(); i++) {
        ...
        if (id / numPerConc == getId() - 2 - CREATE_NUM) {
            ...
            /*
            if (int(m["step"]) % 2 == 0 && int(m["sender"]) / numPerConc == getId() - 2 - CREATE_NUM) {
